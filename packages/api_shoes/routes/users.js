...
// ✅ Register user
router.post('/register', (req, res) => {
  const { name, email, password, phone, user_image } = req.body;
  ...
  db.query(checkSql, [email, phone], (checkErr, checkResult) => {
      ...
      const insertSql = 'INSERT INTO Users (name, email, password, phone, user_image) VALUES (?, ?, ?, ?, ?)';
      db.query(insertSql, [name, email, password, phone, user_image], (insertErr, insertResult) => {
          if (insertErr) {
...
router.get('/:id', (req, res) => {
  ...
  db.query('SELECT user_id, name, email, phone, role, user_image FROM Users WHERE user_id = ?', [id], (err, results) => {
...
router.put('/:id', (req, res) => {
  ...
  const sql = 'UPDATE Users SET name = ?, email = ?, password = ?, phone = ?, role = ?,user_image = ? WHERE user_id = ?';
  db.query(sql, [name, email, password, phone, role, user_image, id], (err, result) => {
